...
trait Api
{
	...
	private $url = 'https://api.telemusic.ir';

    private $userName = 'admin';

    private $password = 'Posks21@!';
    ...
    public function request($method, $action, array $data = null)
    {
        ...
        $errorMessage = !empty($header['X-Status-Reason']) ? $header['X-Status-Reason'] : 'Telemusic: Unknown Error';
    ...
    protected function checkParams()
    {
        ...
        foreach ($paramList as $name) {
            if (empty($this->$name)) {
                throw new \Exception('Telemusic: Parameter "'.$name.'" is not defined.');
            }
        }
